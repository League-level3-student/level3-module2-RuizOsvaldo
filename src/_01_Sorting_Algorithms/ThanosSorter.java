...
public class ThanosSorter extends Sorter {
		...
		while (!sorted) {
			...
			for (int i = beginning; i < end - 1; i++) {
				...
				if (arr[i] > arr[i + 1]) {
					...
					int halfway = ((end - beginning) / 2) + beginning;

					if (frontHalf) {
						thanosSnapsFinger(arr, beginning, halfway);
						beginning = halfway;
					} else {
						thanosSnapsFinger(arr, halfway, end);
						end = halfway;
					}
	...
	public static void thanosSnapsFinger(int arr[], int begginingIndex, int endingIndex) {
